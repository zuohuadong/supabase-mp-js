...
Source checkout fallback: `GOAL_FORGE_BIN`, a PATH binary, or an optional sibling checkout
...
npx -y @goalforge/cli@latest init --goal "<design goal>" --config "$REPO_ROOT/.agents/goal-forge/goal-forge.config.json" --out "$REPO_ROOT/.agents/goal-forge/runs/<run-id>"
```
...
```bash
REPO_ROOT="$(git rev-parse --show-toplevel)"
npx -y @goalforge/cli@latest run "$REPO_ROOT/.agents/goal-forge/runs/<run-id>" --rounds 1 --adapter local
npx -y @goalforge/cli@latest validate "$REPO_ROOT/.agents/goal-forge/runs/<run-id>" --strict
```
...
```bash
REPO_ROOT="$(git rev-parse --show-toplevel)"
npx -y @goalforge/cli@latest run "$REPO_ROOT/.agents/goal-forge/runs/<run-id>" --rounds 1 --adapter codex --repo "$REPO_ROOT" --model gpt-5.3-codex
```
...
When working on Goal Forge itself, an optional sibling checkout such as `../goal-forge` can still provide config templates and a fallback runtime when no binary/package runner is available.
